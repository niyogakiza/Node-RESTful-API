...
handlers._checks.post = function(data, callback){
    // Validate inputs
    let protocol = typeof(data.payload.protocol) === 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1
        ? data.payload.protocol
        : false;
    let url = typeof(data.payload.url) === 'string' && data.payload.url.trim().length > 0
        ? data.payload.url.trim()
        : false;
    let method = typeof(data.payload.method) === 'string' && ['post', 'get', 'put', 'delete'].indexOf(data.payload.method) > -1
        ? data.payload.method
        : false;
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if(!err && userData){
                        ...
                        if(userChecks.length < config.maxChecks){
                            ...
                            // create the check object include the user's phone
                            let checkObject = {
                                'id': checkId,
                                'userPhone': userPhone,
                                'protocol': protocol,
                            ...
                            callback(400, {'Error': 'The user already has the maximum  number of a checks ('+config.maxChecks+')'});
                        }
                    } else {
                        callback(403);
                    }
...
handlers._checks.get = function(data, callback){
    // Check if the id numbers is valid
    let id = typeof(data.queryStringObject.id) === 'string' && data.queryStringObject.id.trim().length === 20
        ? data.queryStringObject.id.trim()
        : false;
    if(id){
        ...
        _data.read('checks', id, function (err, checkData) {
            if(!err && checkData){
                ...
                // Verify that the given token is valid and belongs to the user that created check
